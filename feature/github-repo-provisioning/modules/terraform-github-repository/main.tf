...
resource "github_repository_environment" "environment" {
  ...
  dynamic "deployment_branch_policy" {
    # Only create this block if protected_branches is explicitly set to true
    # If false or omitted, no block is created (any branch can deploy)
    for_each = try(each.value.deployment_branch_policy.protected_branches, false) == true ? [1] : []
